...
<div class="members-grid" v-if="currentMembers.length > 0">
  <div
    ...
    :key="member._id"
    class="member-card"
    :class="{ 'president-secretary-card': activetab === 'president_secretary_members' }"
  >
    <div class="member-header">
      <div class="member-avatar" :class="getAvatarClass()">
    ...
    <div class="member-info">
      ...
      <template v-if="!isEditingMember[index]">
        <!-- President & Secretary Display -->
        <template v-if="activetab === 'president_secretary_members'">
          ...
          <div class="position-section">
            <div class="position-label president-label">
              <i class="bi bi-person-badge"></i>
              President
            </div>
            <h4 class="member-name">{{ member.president || 'No President' }}</h4>
          </div>
          
          <!-- Secretary Section -->
          <div class="position-section secretary-section">
            <div class="position-label secretary-label">
              <i class="bi bi-person-badge"></i>
              Secretary
            </div>
            <h4 class="member-name">{{ member.secretary || 'No Secretary' }}</h4>
          </div>
        </template>

        <!-- Regular member display for other tabs -->
        <template v-else>
          <h4 class="member-name">{{ member.name || 'Unnamed Member' }}</h4>
      ...
      <div v-else class="edit-form">
        <template v-if="activetab === 'president_secretary_members'">
          <!-- President & Secretary Edit Form - REQUIRED FIELDS ONLY -->
          <input
            v-model="safeEditData(index).year"
            placeholder="Year *"
            class="edit-input"
            type="number"
            required
            :class="{ 'error': !safeEditData(index).year }"
          />
          <span class="error-message" v-if="!safeEditData(index).year">Year is required</span>

          <input
            v-model="safeEditData(index).president"
            placeholder="President Name *"
            class="edit-input"
            required
            :class="{ 'error': !safeEditData(index).president }"
          />
          <span class="error-message" v-if="!safeEditData(index).president">President name is required</span>

          <input
            v-model="safeEditData(index).secretary"
            placeholder="Secretary Name *"
            class="edit-input"
            required
            :class="{ 'error': !safeEditData(index).secretary }"
          />
          <span class="error-message" v-if="!safeEditData(index).secretary">Secretary name is required</span>
        </template>
        <template v-else>
          <!-- Regular Member Edit Form -->
          <input
            v-model="safeEditData(index).name"
            placeholder="Name *"
            class="edit-input"
            required
          />
          <input
            ...
            v-model="safeEditData(index).id"
            placeholder="ID *"
            class="edit-input"
          />
          <input
            v-model="safeEditData(index).email"
            placeholder="Email"
            class="edit-input"
            type="email"
          />
          <input
            v-model="safeEditData(index).address"
            placeholder="Address *"
            class="edit-input"
            required
          />
          <input
            v-model="safeEditData(index).phone"
            placeholder="Phone"
            class="edit-input"
          />
          <input
            v-if="activetab !== 'executive_members'"
            v-model="safeEditData(index).state"
            placeholder="State *"
            class="edit-input"
          />
          <input
            v-if="activetab !== 'executive_members'"
            v-model="safeEditData(index).status"
            placeholder="Status"
            class="edit-input"
          />
        </template>
        ...
        <div class="edit-actions">
          <button
            ...
            :disabled="!isFormValid(safeEditData(index))"
          >
          ...
          <div v-show="['president_secretary_members'].includes(activetab)">
            ...
            <div v-if="HidePresidentAndSecretaryMemberForm" class="hmf-modal">
              <PresidentAndSecretaryForm
                @refreshPresidentSecretary="fetchPresidentSecretaryMembers"
...
export default {
  ...
  computed: {
    currentMembers() {
      switch (this.activetab) {
        ...
        case "executive_members":
          ...
          case "president_secretary_members":
        return this.filteredPresidentSecretaryMembers;
        default:
          return [];
      }
    },

    //filter life fellow members
    ...
    // filter president and secretary members
    filteredPresidentSecretaryMembers() {
    if (this.searchQuery) {
      return this.presidentSecretaryMembers.filter(
        (record) =>
          record.year.toString().includes(this.searchQuery) ||
          record.president.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
          record.secretary.toLowerCase().includes(this.searchQuery.toLowerCase())
      );
    }
    return this.presidentSecretaryMembers;
  },
    paginatedMembers() {
  ...
  methods: {
    // Add this method to safely access editMemberData
safeEditData(index) {
  // Ensure editMemberData array exists
  if (!this.editMemberData || !Array.isArray(this.editMemberData)) {
    this.editMemberData = [];
  }
  
  // If the index doesn't exist or is undefined, initialize it
  if (!this.editMemberData[index]) {
    // Get the current member data to initialize with
    const currentMember = this.currentMembers && this.currentMembers[index] 
      ? { ...this.currentMembers[index] } 
      : {};
    
    // Use Vue.set to ensure reactivity
    this.$set(this.editMemberData, index, currentMember);
  }
  
  return this.editMemberData[index];
},
    getAvatarClass() {
    if (this.activetab === 'president_secretary_members') {
      return 'avatar-president-secretary';
    }
    return '';
  },
    getMemberIcon() {
    const icons = {
      honorary_members: 'bi bi-award',
      life_members: 'bi bi-person-check',
      ordinary_members: 'bi bi-people',
      life_fellow_members: 'bi bi-person-heart',
      executive_members: 'bi bi-person-badge',
      president_secretary_members: 'bi bi-calendar-check' // Different icon for president/secretary
    };
    return icons[this.activetab] || 'bi bi-person';
  },
  ...
  async deleteMember() {
    ...
    try {
      ...
      if (this.activetab === "life_fellow_members") {
        ...
        response = await axios.delete(
          `http://localhost:3000/presidentSecretary/delete/${memberId}`
        );
      } else {
      ...
      this.showSuccessToast("Record deleted successfully from database");
    } catch (error) {
      console.error("Error deleting record:", error);
      this.showErrorToast("Error deleting record from database");
      this.isDeleting = false;
    }
  },
...
exportToExcel() {
  ...
  switch (this.activetab) {
    ...
    case "executive_members":
      ...
      break;
    case "president_secretary_members":
      sheetName = "President and Secretary Members";
      break;
    default:
  ...
  // Clean data — remove _id, __v, createdAt, updatedAt
...
getPageTitle(tab) {
  const titles = {
    ...
    president_secretary_members: "President & Secretary",
...
getSectionTitle(tab) {
  const titles = {
    ...
    life_fellow_members: "Life Fellow Members",
    executive_members: "Executive Members",
    president_secretary_members: "President & Secretary Members",
  };
...
getSectionSubtitle(tab) {
  const subtitles = {
    ...
    president_secretary_members:
      "National College of Chest Physicians - Presidents & Secretaries Over the Years",
  };
...
// Add this new method to initialize edit data
initializeEditData() {
  try {
    ...
    if (this.currentMembers && Array.isArray(this.currentMembers) && this.currentMembers.length > 0) {
      // Initialize with proper reactive arrays
      this.isEditingMember = this.currentMembers.map(() => false);
      this.editMemberData = this.currentMembers.map(member => {
        if (member && typeof member === 'object') {
          return JSON.parse(JSON.stringify(member)); // Deep clone
        }
        return {};
      });
    }
...
toggleEditMember(index) {
  // Safety checks
  if (index < 0 || index >= this.currentMembers.length) {
    console.warn('Invalid index for toggleEditMember:', index);
    return;
  }

  // Ensure arrays exist
  if (!Array.isArray(this.isEditingMember)) {
    this.isEditingMember = [];
  }
  if (!Array.isArray(this.editMemberData)) {
    this.editMemberData = [];
  }

  // Create new arrays to ensure reactivity
  const newEditingState = [...this.isEditingMember];
  newEditingState[index] = !newEditingState[index];
  this.isEditingMember = newEditingState;

  // Reset edit data if canceling
  if (!newEditingState[index]) {
    const newEditData = [...this.editMemberData];
    const currentMember = this.currentMembers[index];
    if (currentMember) {
      newEditData[index] = { ...currentMember };
    }
    this.editMemberData = newEditData;
  }
},

// Update the saveEditMember method
async saveEditMember(index, memberId) {
  const updatedData = this.safeEditData(index);
  ...
  try {
    ...
    switch (this.activetab) {
      ...
      case "president_secretary_members":
        payload = {
          ...
          president: updatedData.president || '',
          secretary: updatedData.secretary || ''
        };
        apiURL = `http://localhost:3000/presidentSecretary/update/${memberId}`;
        break;
    ...
    this.showSuccessToast("Record updated successfully in database");
  } catch (error) {
    console.error("Error updating record:", error);
    this.showErrorToast("Error updating record in database");
  }
},

  // Updated validation method - REQUIRED FIELDS ONLY
isFormValid(memberData) {
  if (!memberData) return false;
  
  if (this.activetab === 'president_secretary_members') {
    return memberData.year && memberData.president && memberData.secretary;
  } else {
  ...
  async refreshCurrentData() {
    switch (this.activetab) {
      ...
      case "ordinary_members":
        ...
        break;
      case "president_secretary_members":
        await this.fetchPresidentSecretaryMembers();
        break;
    }
  },
    ...
    // Life Members Method
// Fix the fetch method
async fetchPresidentSecretaryMembers() {
  try {
    const response = await axios.get(
      "http://localhost:3000/api/president-secretary"
    );
    
    console.log("President Secretary API Response:", response.data);
    
    // Your API returns direct array, not nested in data property
    if (Array.isArray(response.data)) {
      this.presidentSecretaryMembers = response.data;
    } else {
      this.presidentSecretaryMembers = [];
    }
    
    // Initialize editing arrays
    this.isEditingMember = Array(this.presidentSecretaryMembers.length).fill(false);
    this.editMemberData = JSON.parse(JSON.stringify(this.presidentSecretaryMembers));
    
    console.log("President and secretary data fetched successfully:", this.presidentSecretaryMembers.length, "records");
  } catch (error) {
    console.error("Error Fetching President Secretary Data:", error);
    this.showErrorToast("Failed to fetch president and secretary data");
    this.presidentSecretaryMembers = [];
  }
},
  ...
  // Add this watcher to watch for currentMembers changes
watch: {
  currentMembers: {
    handler(newMembers) {
      this.initializeEditData();
  ...
  async mounted() {
    ...
    await Promise.all([
      ...
      this.fetchPresidentSecretaryMembers(),
    ]);
...
/* President & Secretary Specific Styles */
.avatar-president-secretary {
  background: linear-gradient(135deg, #667eea, #764ba2);
}
