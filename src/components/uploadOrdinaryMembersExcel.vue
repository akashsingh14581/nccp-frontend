    ...
    <div class="upload-header">
      <div class="header-content">
        ...
        <div class="header-text">
          <h1>Upload Ordinary Members</h1>
          <p>Import Ordinary Members from Excel files (.xls, .xlsx)</p>
        </div>
      </div>
    ...
    <div class="upload-card">
      ...
      <div class="upload-requirements" v-if="!selectedFile">
        ...
        <ul>
          ...
          <li><i class="bi bi-check-circle"></i> Required columns: ID, Name, Address</li>
        </ul>
      </div>
    </div>

    <!-- Data Preview -->
    <div v-if="ordinaryMembers.length > 0" class="preview-section">
      <div class="preview-header">
        <div class="preview-info">
          ...
          <div class="preview-stats">
            <div class="stat">
              <span class="stat-number">{{ ordinaryMembers.length }}</span>
      ...
      <div class="table-container">
        <div class="table-responsive">
          <table class="data-table">
            <thead>
              <tr>
                <th v-for="(value, key) in ordinaryMembers[0]" :key="key" class="column-header">
                  <div class="header-content">
                    <span>{{ formatHeader(key) }}</span>
                    <i class="bi bi-filter" v-if="key !== 'actions'"></i>
                  </div>
                </th>
            ...
            <tbody>
              <tr v-for="(row, index) in paginatedData" :key="index"
                ...
                <td v-for="(value, key) in row" :key="key" :class="getCellClass(key, value)">
                  <template v-if="key === 'status'">
                    <span class="status-badge" :class="getStatusClass(value)">{{ value || 'Active' }}</span>
                  </template>
                  <template v-else-if="key === 'email' && value">
                    <a :href="`mailto:${value}`" class="email-link">{{ value }}</a>
                  </template>
                  <template v-else-if="key === 'phone' && value">
        ...
        <div v-if="ordinaryMembers.length === 0" class="empty-table">
        ...
        <!-- Pagination -->
        <div class="pagination" v-if="ordinaryMembers.length > 0">
          <div class="pagination-info">
            Showing {{ startIndex + 1 }} to {{ endIndex }} of {{ ordinaryMembers.length }} entries
          </div>
    ...
    <div v-if="showEditModal" class="modal-overlay" @click="closeEditModal">
      <div class="modal-content large" @click.stop>
        <div class="modal-header">
          <h3>{{ editingRowIndex !== null ? 'Edit Member' : 'Add New Member' }}</h3>
          ...
          <form @submit.prevent="saveEditedRow" class="edit-form">
            <div class="form-grid">
              <div class="form-group">
                ...
                <input id="edit-id" type="text" v-model="editingRow.id" required :class="{ 'error': !editingRow.id }"
                  placeholder="Enter member ID" />
                <span class="error-message" v-if="!editingRow.id">Member ID is required</span>
              </div>

              <div class="form-group">
                <label for="edit-name">Full Name *</label>
                <input id="edit-name" type="text" v-model="editingRow.name" required
                  :class="{ 'error': !editingRow.name }" placeholder="Enter full name" />
                <span class="error-message" v-if="!editingRow.name">Name is required</span>
              </div>

              <div class="form-group">
                <label for="edit-email">Email</label>
                <input id="edit-email" type="email" v-model="editingRow.email"
                  :class="{ 'error': editingRow.email && !isValidEmail(editingRow.email) }"
                  placeholder="Enter email address" />
                <span class="error-message" v-if="editingRow.email && !isValidEmail(editingRow.email)">
              ...
              <div class="form-group">
                ...
                <input id="edit-phone" type="tel" v-model="editingRow.phone" placeholder="Enter phone number" />
              </div>

              <div class="form-group full-width">
                <label for="edit-address">Address *</label>
                <textarea id="edit-address" v-model="editingRow.address" required
                  :class="{ 'error': !editingRow.address }" placeholder="Enter complete address" rows="3"></textarea>
                <span class="error-message" v-if="!editingRow.address">Address is required</span>
              </div>

              <div class="form-group">
                <label for="edit-state">State</label>
                <input id="edit-state" type="text" v-model="editingRow.state" placeholder="Enter state/province" />
              </div>

              <div class="form-group">
                <label for="edit-status">Status</label>
                <select id="edit-status" v-model="editingRow.status">
      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <div class="delete-warning">
            ...
            <p>This action cannot be undone. The following member will be permanently removed:</p>
            <div class="member-info">
              <strong>{{ memberToDelete?.name }}</strong>
              <span>ID: {{ memberToDelete?.id }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <div class="guide-section">
            ...
            <ul>
              <li><strong>id:</strong> Member ID (Required)</li>
...
export default {
  ...
  computed: {
    validRecords() {
      return this.ordinaryMembers.filter(row =>
        row.id && row.name && row.address
      ).length;
    },
    invalidRecords() {
      return this.ordinaryMembers.length - this.validRecords;
    },
    paginatedData() {
      const start = (this.currentPage - 1) * this.pageSize;
      const end = start + this.pageSize;
      return this.ordinaryMembers.slice(start, end);
    },
    totalPages() {
      return Math.ceil(this.ordinaryMembers.length / this.pageSize);
    },
    visiblePages() {
      const maxVisible = 5;
    ...
    endIndex() {
      return Math.min(this.startIndex + this.pageSize, this.ordinaryMembers.length);
    },
    isFormValid() {
      return this.editingRow.id && this.editingRow.name && this.editingRow.address;
    }
  },
  methods: {
     goToDashboard() {
    ...
    handleFileUpload(event) {
      ...
      reader.onload = (e) => {
        try {
          ...
          this.ordinaryMembers = jsonData;
          this.isUploading = false;
          this.uploadProgress = 100;
          this.showSuccessToast(`Successfully imported ${jsonData.length} records`);
    ...
    async saveDataToDB() {
      ...
      try {
        const formattedData = this.ordinaryMembers.map((member) => ({
        ...
        this.showSuccessToast(`Successfully saved ${formattedData.length} members to database`);
    ...
    editRow(index) {
      ...
      this.editingRow = { ...this.ordinaryMembers[actualIndex] };
    ...
    saveEditedRow() {
      ...
      if (this.editingRowIndex !== null) {
        // Update existing row
        this.ordinaryMembers[this.editingRowIndex] = { ...this.editingRow };
        this.showSuccessToast('Member updated successfully');
      } else {
        // Add new row
        this.ordinaryMembers.unshift({ ...this.editingRow });
        this.showSuccessToast('Member added successfully');
      }
    ...
    deleteRow(index) {
      const actualIndex = this.startIndex + index;
      this.memberToDelete = this.ordinaryMembers[actualIndex];
    ...
    confirmDelete() {
      if (this.deleteIndex !== null) {
        this.ordinaryMembers.splice(this.deleteIndex, 1);
    ...
    getCellClass(key, value) {
      const classes = [];
      if (key === 'id' && !value) classes.push('missing-required');
      if (key === 'name' && !value) classes.push('missing-required');
      if (key === 'address' && !value) classes.push('missing-required');
      if (key === 'email' && value && !this.isValidEmail(value)) classes.push('invalid-email');
      return classes.join(' ');
    },
    ...
    hasDataIssue(row) {
      return !row.id || !row.name || !row.address;
    },
    isValidEmail(email) {
...
<style scoped>
/* Exact same styles as Life Members component */
