<template>
  <div class="excel-upload-container">
    ...
    <div class="upload-header">
      <div class="header-content">
        ...
        <div class="header-text">
          <h1>Upload Honorary Member Data</h1>
          <p>Import Honorary Members from Excel files (.xls, .xlsx)</p>
        </div>
      </div>
    ...
    <div class="upload-card">
      ...
      <div class="upload-requirements" v-if="!selectedFile">
        ...
        <ul>
          ...
          <li><i class="bi bi-check-circle"></i> Required columns: ID, Name</li>
        </ul>
      </div>
    </div>

    <!-- Data Preview -->
    <div v-if="honoraryMembers.length > 0" class="preview-section">
      <div class="preview-header">
        <div class="preview-info">
          ...
          <div class="preview-stats">
            <div class="stat">
              <span class="stat-number">{{ honoraryMembers.length }}</span>
      ...
      <div class="table-container">
        <div class="table-responsive">
          <table class="data-table">
            <thead>
              <tr>
                <th v-for="(value, key) in honoraryMembers[0]" :key="key" class="column-header">
                  <div class="header-content">
              ...
              <tr v-for="(row, index) in paginatedData" :key="index" 
                ...
                <td v-for="(value, key) in row" :key="key" :class="getCellClass(key, value)">
                  <template v-if="key === 'status'">
                    <span class="status-badge" :class="getStatusClass(value)">{{ value || 'Unknown' }}</span>
                  </template>
        ...
        <div v-if="honoraryMembers.length === 0" class="empty-table">
        ...
        <!-- Pagination -->
        <div class="pagination" v-if="honoraryMembers.length > 0">
          <div class="pagination-info">
            Showing {{ startIndex + 1 }} to {{ endIndex }} of {{ honoraryMembers.length }} entries
          </div>
    ...
    <div v-if="showEditModal" class="modal-overlay" @click="closeEditModal">
      <div class="modal-content large" @click.stop>
        <div class="modal-header">
          <h3>{{ editingRowIndex !== null ? 'Edit Honorary Member' : 'Add New Honorary Member' }}</h3>
      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <div class="delete-warning">
            ...
            <h4>Are you sure you want to delete this honorary member?</h4>
    ...
    <div v-if="showHelp" class="modal-overlay" @click="showHelp = false">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3>Honorary Members Excel Upload Guide</h3>
...
export default {
  ...
  computed: {
    validRecords() {
      return this.honoraryMembers.filter(row => 
        row.id && row.name
      ).length;
    },
    invalidRecords() {
      return this.honoraryMembers.length - this.validRecords;
    },
    paginatedData() {
      const start = (this.currentPage - 1) * this.pageSize;
      const end = start + this.pageSize;
      return this.honoraryMembers.slice(start, end);
    },
    totalPages() {
      return Math.ceil(this.honoraryMembers.length / this.pageSize);
    },
    visiblePages() {
      const maxVisible = 5;
    ...
    endIndex() {
      return Math.min(this.startIndex + this.pageSize, this.honoraryMembers.length);
    },
    isFormValid() {
      return this.editingRow.id && this.editingRow.name;
    }
  },
  methods: {
       goToDashboard() {
    ...
    handleFileUpload(event) {
      ...
      reader.onload = (e) => {
        try {
          ...
          this.honoraryMembers = jsonData;
          this.isUploading = false;
          this.uploadProgress = 100;
          this.showSuccessToast(`Successfully imported ${jsonData.length} honorary member records`);
    ...
    async saveDataToDB() {
      if (this.honoraryMembers.length === 0) {
      ...
      try {
        const formattedData = this.honoraryMembers.map((honoraryMember) => ({
          id: honoraryMember['id']?.toString() || '',
          name: honoraryMember['name'] || '',
          address: honoraryMember['address'] || '',
          state: honoraryMember['state'] || '',
          status: honoraryMember['status'] || 'Active',
          phone: honoraryMember['phone']?.toString() || '',
          email: honoraryMember['email'] || '',
        }));
        ...
        console.log('Honorary data saved to DB:', response.data);
        this.showSuccessToast(`Successfully saved ${formattedData.length} honorary members to database`);
        
      } catch (error) {
        console.error('Error saving honorary data to DB:', error);
    ...
    editRow(index) {
      ...
      this.editingRow = { ...this.honoraryMembers[actualIndex] };
    ...
    saveEditedRow() {
      ...
      if (this.editingRowIndex !== null) {
        // Update existing row
        this.honoraryMembers[this.editingRowIndex] = { ...this.editingRow };
        this.showSuccessToast('Honorary member updated successfully');
      } else {
        // Add new row
        this.honoraryMembers.unshift({ ...this.editingRow });
        this.showSuccessToast('Honorary member added successfully');
      }
    ...
    deleteRow(index) {
      const actualIndex = this.startIndex + index;
      this.memberToDelete = this.honoraryMembers[actualIndex];
    ...
    confirmDelete() {
      if (this.deleteIndex !== null) {
        this.honoraryMembers.splice(this.deleteIndex, 1);
        this.showSuccessToast('Honorary member deleted successfully');
...
<style scoped>
/* Exact same styles as Life Fellow Members component */
