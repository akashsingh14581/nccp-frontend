  ...
  data() {
    return {
      ...
      contentTabs: [
        ...
        { id: "events", name: "Create Events", icon: "bi bi-images" },
      ],
      // Existing data
      lifeMembers: [],
  ...
  methods: {
    ...
    async saveEditMember(index, memberId) {
      ...
      try {
        ...
        if (this.activetab === "life_members") {
          // Use your specific API endpoint for life members
          response = await axios.put(
            `http://localhost:3000/fellowmember/editFellowMember/${memberId}`,
            {
              id: updatedData.id,
          ...
          // Use existing endpoint for honorary members
          response = await axios.put(
