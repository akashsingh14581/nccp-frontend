<template>
  <div class="excel-upload-container">
    ...
    <div class="upload-header">
      <div class="header-content">
        ...
        <div class="header-text">
          <h1>Upload Member Data</h1>
          <p>Import Life Fellow Members from Excel files (.xls, .xlsx)</p>
        </div>
      </div>
    ...
    <!-- Data Preview -->
    <div v-if="lifeMembers.length > 0" class="preview-section">
      <div class="preview-header">
        <div class="preview-info">
          ...
          <div class="preview-stats">
            <div class="stat">
              <span class="stat-number">{{ lifeMembers.length }}</span>
      ...
      <div class="table-container">
        <div class="table-responsive">
          <table class="data-table">
            <thead>
              <tr>
                <th v-for="(value, key) in lifeMembers[0]" :key="key" class="column-header">
                  <div class="header-content">
        ...
        <div v-if="lifeMembers.length === 0" class="empty-table">
        ...
        <!-- Pagination -->
        <div class="pagination" v-if="lifeMembers.length > 0">
          <div class="pagination-info">
            Showing {{ startIndex + 1 }} to {{ endIndex }} of {{ lifeMembers.length }} entries
          </div>
...
export default {
  ...
  computed: {
    validRecords() {
      return this.lifeMembers.filter(row => 
        row.id && row.name && row.address
      ).length;
    },
    invalidRecords() {
      return this.lifeMembers.length - this.validRecords;
    },
    paginatedData() {
      const start = (this.currentPage - 1) * this.pageSize;
      const end = start + this.pageSize;
      return this.lifeMembers.slice(start, end);
    },
    totalPages() {
      return Math.ceil(this.lifeMembers.length / this.pageSize);
    },
    visiblePages() {
      const maxVisible = 5;
    ...
    endIndex() {
      return Math.min(this.startIndex + this.pageSize, this.lifeMembers.length);
    },
    isFormValid() {
  ...
  methods: {
    ...
    async saveDataToDB() {
      ...
      try {
        const formattedData = this.lifeMembers.map((lifeMember) => ({
          id: lifeMember['id']?.toString() || '',
          name: lifeMember['name'] || '',
          address: lifeMember['address'] || '',
          state: lifeMember['state'] || '',
          status: lifeMember['status'] || 'Active',
          phone: lifeMember['phone']?.toString() || '',
          email: lifeMember['email'] || '',
        }));

        console.log('Formatted Data:', formattedData);

        const response = await axios.post(`${API_BASE_URL}/fellowmember/uploadfellowmember`, {
    ...
    editRow(index) {
      ...
      this.editingRow = { ...this.lifeMembers[actualIndex] };
    ...
    saveEditedRow() {
      ...
      if (this.editingRowIndex !== null) {
        // Update existing row
        this.lifeMembers[this.editingRowIndex] = { ...this.editingRow };
        this.showSuccessToast('Member updated successfully');
      } else {
        // Add new row
        this.lifeMembers.unshift({ ...this.editingRow });
    ...
    deleteRow(index) {
      const actualIndex = this.startIndex + index;
      this.memberToDelete = this.lifeMembers[actualIndex];
    ...
    confirmDelete() {
      if (this.deleteIndex !== null) {
        this.lifeMembers.splice(this.deleteIndex, 1);
...
<style scoped>
/* Previous styles remain the same, adding new styles for edit and delete functionality */
